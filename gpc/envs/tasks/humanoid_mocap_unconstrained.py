"""Humanoid mocap tracking task (Unitree G1) — same as zoo-rob/traj_opt/tasks/humanoid_mocap_unconstrained.py.

Uses LocoMuJoCo retargeted mocap; cost is configuration, foot pose, and control tracking.
Exposes finite u_min/u_max for GPC from original actuator limits.
"""
...
class HumanoidMocapUnconstrained(Task):
    ...
    def __init__(
        ...
        mj_model.actuator_ctrlrange[:] = [-np.inf, np.inf]
        ...
        # Renderer uses MuJoCo model; ensure it's available (parent may use .model for MJX)
        if not hasattr(self, "mj_model"):
            self.mj_model = mj_model

        # GPC expects finite action bounds; use original actuator limits
        self.u_min = jnp.array(self.lb)
        self.u_max = jnp.array(self.ub)
        ...
        reference_full = npz_file["qpos"]
        self.reference = jnp.array(reference_full[start:, :])
        self.reference_fps = float(npz_file["frequency"])

        # Precompute reference foot positions/orientations for the SLICED reference
        # so that index i corresponds to self.reference[i].
        mj_data = mujoco.MjData(mj_model)
        reference_sliced = reference_full[start:]
        n_frames = len(reference_sliced)
        ...
        for i in range(n_frames):
            mj_data.qpos[:] = reference_sliced[i]
    ...
    def _get_reference_configuration(self, t: jax.Array) -> jax.Array:
        """Get the reference qpos at time t (seconds)."""
    ...
    def _get_reference_foot_data(self, t: jax.Array) -> tuple[jax.Array, ...]:
        """Get reference foot positions and quaternions at time t."""
        i = jnp.int32(t * self.reference_fps)
        i = jnp.clip(i, 0, self.reference.shape[0] - 1)
        return (
            self.ref_left_pos[i],
    ...
    def _get_foot_orientation_errors(
        ...
        """Get orientation errors (quat_sub returns 3D rotation vector)."""
    ...
    def running_cost(self, state: mjx.Data, control: jax.Array) -> jax.Array:
        """Running cost: config + foot pose + control tracking."""
    ...
    def terminal_cost(self, state: mjx.Data) -> jax.Array:
        """Terminal cost (same terms, no control)."""
        q_ref = self._get_reference_configuration(state.time)
        q = state.qpos
        q_err = self.cost_weights * (q - q_ref)
        configuration_cost = jnp.sum(jnp.square(q_err))
    ...
    def domain_randomize_data(
        ...
        """Perturb base position and velocity."""
